            ...
            <div class="col">
                <div class="card bg-default shadow">
                    <div class="card-header bg-transparent border-0">
                        @if(Route::currentRouteName() == 'account.orders')
                        <h3 class="text-white mb-0">Orders</h3>
                        @elseif(Route::currentRouteName() == 'account.pending-orders')
                            <h3 class="text-white mb-0">Pending orders</h3>
                        @elseif(Route::currentRouteName() == 'account.complete-orders')
                            <h3 class="text-white mb-0">Complete orders</h3>
                        @endif
                    </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="media-body">
                                                ...
                                                <span style="display: block;">Seller - {{ $product->productId->user->name }}</span>
                                            </div>
                                        </td>
                                        <td>
                                        ...
                                        <td data-id="{{ $product->id }}">
                                            @if(!$product->tracker_id && $order->sale)
                                            ...
                                            @else
                                                {{ $product->tracker_id ? $product->tracker_id : '-' }}
                                            @endif
                                        </td>

                                        <td class="text-right">
                                            @if($product->status == 'Pending')
                                                <a data-id="{{ $product->id }}" href="javascript:void(0);" class="btn-confirm" data-status="Success">Confirm</a>
                                            @endif

                                                @if($product->status == 'Created' && $order->sale)
                                                    <a href="javascript:void(0);" data-id="{{ $product->id }}" class="btn-confirm" data-status="Pending">Set status pending</a>
                                                @endif
    ...
    <script>
        $('.btn-confirm').click(function () {
            ...
            $.ajax({
                ...
                data: {id, '_token': $('#token').attr('content'), status},
                success:function (result) {
                    window.location.href = '/{{ request()->path() }}?success=1'
                }
            })
