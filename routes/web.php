...
Route::group(['middleware' => ['checkUser']], function () {
     ...
     * Working sample
     */
    Route::get('/create-payment-account', [\App\Http\Controllers\PaymentController::class, 'createPaymentAccount'])->name('createPaymentAccount');
    Route::get('/continue-payment-account', [\App\Http\Controllers\PaymentController::class, 'continuePaymentAccount'])->name('continuePaymentAccount');
    Route::any('/stripe/refresh', [\App\Http\Controllers\PaymentController::class, 'stripeConnectRefresh'])->name('stripeConnectRefresh');
    Route::any('/stripe/return', [\App\Http\Controllers\PaymentController::class, 'stripeConnectReturn'])->name('stripeConnectReturn');

    /*
     * Testing
     */
//    Route::get('/initiate-connect', [\App\Http\Controllers\PaymentController::class, 'initiateConnect']);
//    Route::get('/laravel-stripe-connect/public/redirect', [\App\Http\Controllers\PaymentController::class, 'redirectConnect']);
//    Route::get('/seller-transfer', [\App\Http\Controllers\PaymentController::class, 'sellerTransfer']);
});
...
Route::post('/cart/remove-item', 'CartController@removeItem');
Route::post('/cart/clear', 'CartController@clear');
Route::post('/cart/change-count-item', 'CartController@changeCountItem');

Route::match(['get', 'post'], '/delivery', 'CartController@delivery')->name('cart.delivery');
...
Route::group(['middleware' => ['checkUser'], 'prefix' => 'dashboard', 'as' => 'account.', 'namespace' => 'Account'], function () {
    Route::get('/', 'AccountController@home')->name('home');
    Route::get('/orders', 'OrderController@orders')->name('orders');
    Route::get('/pending-orders', 'OrderController@orders')->name('pending-orders');
    Route::get('/complete-orders', 'OrderController@orders')->name('complete-orders');

    Route::post('/change-status-order-product', 'OrderController@changeStatusOrderProduct');

    Route::get('/sales', 'OrderController@sales')->name('sales');

    Route::post('/set-tracker-number', 'OrderController@setTrackerNumber');
});


Route::get('{user_name}/{id}', 'SiteController@product_details')->name('product_details');
